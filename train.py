...
def mixup_data(x, y, lam):
    ...
    '''Compute the mixup image. Return mixed inputs, pairs of targets, and lambda'''
...
class WaveletFusion(nn.Module):
    def __init__(self, wavelet='haar'):
        super(WaveletFusion, self).__init__()
        self.wavelet = wavelet

    def forward(self, x):
            # 小波分解
        coeffs = pywt.dwt2(x.cpu().numpy(), self.wavelet)  # 分解成低频和高频分量
        LL, (LH, HL, HH) = coeffs

            # 转换回 tensor
        LL = torch.tensor(LL, device=x.device).float()
        LH = torch.tensor(LH, device=x.device).float()
        HL = torch.tensor(HL, device=x.device).float()
        HH = torch.tensor(HH, device=x.device).float()

            # 融合策略（简单加权和示例）
        fused = LL + 0.5 * (LH + HL + HH)
...
class WideResNet(nn.Module):
    def __init__(self, depth=28, widen_factor=10, num_classes= 200 , loss_type = 'dist', per_img_std = False, stride = 1 ):
        ...
        self.conv1 = nn.Conv2d(3, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.stn1 = stn.STN(in_chale=16)
        self.stn2 = stn.STN(in_chale=16)
        # 1st block
